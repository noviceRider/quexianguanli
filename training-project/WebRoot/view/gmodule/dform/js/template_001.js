...
$(function(){
	...
	//加载查询条件新增、修改对话框
	$("#queryConditions_html").load("queryconditions_am.html",null,function(){
		//解析页面
		$.parser.parse(this);
		//加载条件类型数据
		$.ajax({
	...
	//加载生成新增界面的新增、修改弹框
	$("#add_html").load("addhtml_am.html",null,function(){
		//解析页面
		$.parser.parse(this);
		//加载新增、修改弹框输入框类型
		$.ajax({
...
function SaveAllData(){
	if($("#fm").form("validate")){
		...
		console.log(JSON.stringify(parserParams));
		var data = parseHtml(parserParams);
		console.log(data);
		//转换为字符串
 ...
 * @param boxId
 */
function addAccess(accessType,boxId) {
	inputId = boxId;
	loadLeftTree();
	//给数据网格加载数据
	console.log();
	var params={categoryCode:'',keyWord:'',typeCode:accessType};
...
function selectLogic() {
	...
	$("#logicCodeId").textbox("setValue",logic.logicCode);
