...
$(function(){
	...
	//数据网格加载数据
	$("#metamanage_dg").datagrid( Common.createDatagridOptionsExt(basePath, "/gmodule_dynamictables_metamanage_findpage.json",135));
...
function queryMetamanage(){
	$("#metamanage_dg").datagrid("reload",{keyWord:$("#keyWord").textbox("getValue")});
}
...
function addMetamanage(){
	...
	isNew=true;
	//设置新增url
	url=basePath+"/gmodule_dynamictables_metamanage_add.json";
...
function modifyMetamanage(){
	...
	isNew=false;
	//设置修改url
	url=basePath+"/gmodule_dynamictables_metamanage_modify.json";
	...
	// 获取需要修改的这条数据
	$.ajax({
		url: basePath + "/gmodule_dynamictables_metamanage_findByTableName.json",
		type: 'post',
		data: rows[0],  
		dataType:"json",
	...
	// 获取需要修改的这条数据对应的字段信息
	$.ajax({
		url: basePath + "/gmodule_dynamictables_metamanage_findColumnByTableName.json",
		type: 'post',
		data: rows[0],  
		dataType:"json",
		success: function(data) {
...
function removeMetamanage(){
	...
	$.messager.confirm("确认", "是否将选择的功能信息从数据库中删除，这一操作是不可逆的，是否继续？", function(r) {
		if (r) {
			$.ajax({
				url : basePath + "/gmodule_dynamictables_metamanage_remove.json",
				type : "post",
				data : tableNames,
				success : function(data) {
					// 更新表格中的数据
...
function validateMetamanage(formId,row) {
	if(formId == "Add"){// 新增验证
		$("#tableNameId").textbox({
			...
			validType:"remoteValid['"+basePath+"/gmodule_dynamictables_metamanage_validatetablename.json','tableName']"
		});
//		$("#entityNameId").textbox({
//			required:true,
//			missingMessage:"实体名不能为空",
//			invalidMessage:"实体名已经存在",
//			validType:"remoteValid['"+basePath+"/gmodule_dynamictables_metamanage_validateentityname.json','entityName']"
//		});
...
//			missingMessage:"表名不能为空",
//			invalidMessage:"表名已经存在",
//			validType:"remoteValid['"+basePath+"/gmodule_dynamictables_metamanage_validatetablename.json','tableName',{selfTableName:'"+row.tableName+"'}]"
//		});
//		$("#entityNameId").textbox({
//			required:true,
//			missingMessage:"实体名不能为空",
//			invalidMessage:"实体名已经存在",
//			validType:"remoteValid['"+basePath+"/gmodule_dynamictables_metamanage_validateentityname.json','entityName',{selfEntityName:'"+row.entityName+"'}]"
//		});
	}
}
...
function createTable() {
	...
	$.messager.confirm("确认", "生成表后，原有的表会被替换，数据将被清除，是否继续？", function(r) {
		if (r) {
			//在数据库生成表
			$.ajax({
				url: basePath + "/gmodule_dynamictables_metamanage_createTable.json",
				type: 'post',
				data: rows[0],  
				dataType:"json",
				success: function(data) {
