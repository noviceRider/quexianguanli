...
$(function(){	
	...
	//页面datagrid加载数据参数
	var queryParams = Common.createDatagridOptionsParams(basePath, "/dataaccess.json", 135, {accessId:"8205",userId:user.userId});
	queryParams.height = tableHeight;
	//数据网格加载
	$("#showFlowTable").datagrid(queryParams);
...
function trackFlow(procDefId,index,procInstId){
	...
	$.ajax({
		...
		success:function(data){
			...
			allShowData[0].assigneeEmpName=$("#showFlowTable").datagrid('getRows')[index].empName;
			$("#detail_task_dg").datagrid("loadData",allShowData);
		}
	});
	$("#processpic").attr("src",basePath+"/app_frame_workflow_trackflow.json?procDefId="+procDefId+"&procInstId="+procInstId+"&t="+new Date().valueOf());
	showFlowInfo(index);
...
function showBusinessInfo(index){
	var row = $("#showFlowTable").datagrid('getRows')[index];
	//存在业务信息关键字则显示，业务信息
	if(row.viewFormKey){
