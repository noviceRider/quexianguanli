...
function toNextPage(index){
	if(index == 1){
		...
		$("#addDivPageStacker").pageStacker("openPage","#addFour");
		$("#addFourPanel").panel('resize',{
			width: "100%",
			height:"100%"
		});
		$("#four").removeClass("uncomplete-bg");
		$("#four").addClass("complete-bg");
		
		
		if($("#evalPicDefDiv").children().length == 0){
			$.ajax({
				url:basePath + "/dataaccess.json",
				type:"post",
				dataType:"json",
				...
				$.ajax({
					...
					success:function(data){
						...
						for(var i=0;i<data.rows.length;i++){
							...
							$("form[id^='form_addImg'] img[alt='"+picType+"']").attr("src","http://localhost:8080/pcms/gmodule_resmanage_filedown.json?id="+data.rows[i].resId);
							
							var imgList=$("form[id^='form_addImg'] img[src^='http://localhost:8080/pcms/gmodule_resmanage_filedown.json?id=']");
							var indexList=[];
							if(imgList!=0){
 ...
 */
function addOneAttribute(attributeData){
	var indexId = attributeData.attrCode.replace(".", '');;
	var newAttributeDiv = '';
	//判断属性  01 唯一属性 02 单选属性  03 多选属性
	if(attributeData.attrType == '01'){
		newAttributeDiv = '<tr><td style="width: 50%; text-align: right;">'
			+attributeData.attrName
			+'&nbsp;</td><td style="width: 50%;"><input type="text" class="easyui-textbox" id='
			+indexId
			+' name='
			+attributeData.attrCode
			+' style="width:240px;"/></td></tr>';
	}else if(attributeData.attrType == '02'){
		newAttributeDiv ='<tr><td style="width: 50%; text-align: right;">'
			+attributeData.attrName
			+'&nbsp;</td><td style="width: 50%;"><select id='
			+indexId
			+' name='
			+attributeData.attrCode
			...
			+',textField: '
			+"'valueText' "
			+'" value="" style="width:240px;"></select></td></tr>';
	}else if(attributeData.attrType == '03'){
		newAttributeDiv = '<tr><td style="width: 50%; text-align: right;">'
			+attributeData.attrName
			+'&nbsp;</td><td style="width: 50%;" id='
			+indexId
			+'>';	
		...
		newAttributeDiv += '</td></tr>';
	}
	$("#goodsAttribute_table").append(newAttributeDiv);
	//赋值
	if(attributeData.attrType == '01'){
...
function addOneImgDiv(evalPicDefItem){
	...
	var divItem = '<form id="form_addImg'+uploadImgNum+'" style="width:20%;float:left;"  class="easyui-form" enctype="multipart/form-data" method="POST"><div style="width:100%;height:100%;">'
		...
		+'<img id="showImg'+uploadImgNum+'" alt="'+evalPicDefItem.picType+'" src="images/def.png" style="width:100%;height:100%;margin-bottom:0;padding-bottom:0;">'
...
function deleteEvalPicDef(index){
	...
	$("#showImg"+index).attr({"src":"images/def.png"});
}
...
function getSelectHtml(rows,type){
	...
	if(type == "shop"){
		for (var i = 0; i < rows.length; i++) {
			if(i == 0){
				html += "<option value=\""+rows[i].catCode+"\"  style=\"height:20px;padding:6px 0 0 8px;;background: url('images/liIcon.png') no-repeat 140px;background-size:20px 20px;\" selected='selected'>"+rows[i].catName+"</option>";
			}else{
				html += "<option value=\""+rows[i].catCode+"\"  style=\"height:20px;padding:6px 0 0 8px;;background: url('images/liIcon.png') no-repeat 140px;background-size:20px 20px;\">"+rows[i].catName+"</option>";
			}
		}
	}else if(type == "brand"){
		for (var i = 0; i < rows.length; i++) {
...
function goodsValid(){	
	$("#goodsName").textbox({
		...
		validType:"remoteValid['" + basePath + "/businesslogic.json', 'goodsName', {logicId: '06.02.05.01',detailCatCode:'"+subCode+"'}]"
	});
...
function perfectOprate(row){
	...
	$("#goodsBrand").combobox("setValue",row.goodsBrand);
...
function stepToGoodsInfo(goodsCatCode){
	...
	$.ajax({
		...
		method:"post",
		success:function(data){
			var comboData=[{dictDesc:"请选择商品来源",dictCode:""}];
	...
	$.ajax({
		...
		success:function(data){
			if(data != null){
				...
				if(rows != null && rows != '' && rows.length != 0){
//					$("#goodsBrand").html(getSelectHtml(rows,"brand"));
					//加载combox数据
					
					$('#goodsBrand').combobox("setValue",rows[0].brandCode);
				}
			}			
		},
		error:function(){
...
function saveGoodsInfo(){//完成
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.resultCode == 1){
				$.messager.alert("提示信息","信息保存成功","info");
				refresh();
				//提交成功刷新数据
				$("#productsTable").datagrid("reload");
				
				//保存属性
				saveGoodsPr(updateGoodsInfo.goodsId);
